// Enhanced Pong Game with all requested features
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    function drawBackground() {
        ...
        for (let i = 0; i < 15; i++) {
            const x = (i * 40 + backgroundOffset * 0.7) % backgroundCanvas.width;
            bgCtx.beginPath();
            bgCtx.moveTo(x, 0);
    ...
    function activatePowerUp(type) {
        switch(type) {
            case "speed":
                ...
                break;
            case "size":
                if (gameMode === "single" || gameMode === "survival") {
                    player.height *= 1.5;
    ...
    function initGameMode() {
        balls.length = 1; // Reset to single ball
        balls[0] = { x: canvas.width / 2, y: canvas.height / 2, r: 8, dx: 4, dy: 4, trail: [], skin: ballSkin };
        
        if (gameMode.includes("survival")) {
            survivalTime = 0;
            // Add multiple balls for survival mode
            for (let i = 0; i < 3; i++) {
                balls.push({
                    x: canvas.width / 2,
                    y: canvas.height / 2,
        ...
        } else if (gameMode.includes("tournament")) {
            tournamentRound = 1;
        }
    }
    ...
    function update() {
        ...
        // AI movement (only if not 2-player mode)
        const is2PlayerMode = gameMode === "local2p" || gameMode === "tournament-2p" || gameMode === "survival-2p";
        if (!is2PlayerMode) {
            const aiCenter = ai.y + ai.height / 2;
            const targetBall = balls[0]; // AI targets first ball
        ...
        if (gameMode.includes("survival")) {
            // Survival mode continues until player misses
        } else if (gameMode.includes("tournament")) {
    ...
    function endGame() {
        ...
        if (gameMode.includes("survival")) {
            ...
            message = tournamentRound > 3 ? `🏆 ${displayName} wins the tournament!` : "😢 Tournament Over!";
        } else {
        ...
        if (playerScore > aiScore || gameMode.includes("survival")) {
    ...
    if (gameModeSelect) {
        gameModeSelect.addEventListener('change', function() {
            ...
            if (is2Player) {
                ...
                document.getElementById('playerNameInput').placeholder = 'Enter Player 1 name';
            } else {
                player2Input.classList.add('hidden');
                document.getElementById('playerNameInput').placeholder = 'Enter your name';
            }
        });
    }
